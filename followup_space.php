...
$sql_gid = "SELECT group_id FROM " .$group_tutor_table. "WHERE user_id = " .$uid. "";
//echo $sql_gid;
$result_gid =  mysql_query($sql_gid);
while ($data = mysql_fetch_array($result_gid)){
	$gid = $data['group_id'];
}
...
{
	echo "<a href=\"followup_edit.php?origin=$origin\">".get_lang("EditGroup")."</a><br/><br/>";
}
...
{
	$sql = "SELECT * FROM ".$task_table." WHERE id='".$_GET['id']."'";
	$result = mysql_query($sql);
	echo "teeeest";
	while ($data = mysql_fetch_array($result)){
		echo $data['title'];
	}
	$sql2 = "SELECT * FROM ".$task_section." WHERE id_followup_task='".$_GET['id']."'";
...
$parameters = array('cidReq' => $_GET['cidReq'], 'origin'=> $_GET['origin'], 'gidReq' =>  $gid);/*$_GET['gidReq']*/
//print_r($parameters);
...
$sql = "SELECT title FROM ".$task_table."WHERE 1";
$result = mysql_query($sql);
$i = 4;
while ($data = mysql_fetch_array($result)){
	$table->set_header($i, $data['title']);
	$table->set_column_filter($i, 'validation');
	$i++;
}

$table->set_column_filter(4, 'validation');
$table->set_column_filter(3, 'email_filter');
$table->set_column_filter(0, 'user_icon_filter');

if(/*GroupManager :: is_self_registration_allowed($_SESSION['_user']['user_id'], $current_group['id'])*/ api_is_allowed_to_edit()/*GroupManager :: is_tutor($_user['user_id'])*/)
$table->display();
else
{
	...
	echo '<br/><br/><br/><div style="width:300; height:180; overflow:auto; border-width: 1px; border-style: solid; border-color: #000000;">
	...
	while ($data = mysql_fetch_array($result)){
		 echo '<tr height="40" valign="middle">
			   <td><a href=./followup_space.php?origin='.$origin.'&amp;uInfo='.$user_id.'&amp;id='.$data['id'].'&amp;task=1>'.$data['title'].'</a></td>
			   <td>'.$data['desc'].'</td>
			   <td>'.$data['lien'].'</td>
...
{
	global $origin;
	return "<a href='../user/userInfo.php?origin=".$origin."&amp;uInfo=".$user_id."'><img src='../img/members.gif' ></a>";
}

function validation($user_id)
{
	global $origin;
	return "<a href='../user/userInfo.php?origin=".$origin."&amp;uInfo=".$user_id."'><img src='../img/valid_na.gif' ></a>";
}
